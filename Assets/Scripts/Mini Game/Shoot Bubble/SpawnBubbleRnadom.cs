...
public class SpawnBubbleRnadom : MonoBehaviour
{
    ...
    void MoveBubble()
    {
        ...
        mousePosition.z = currentBubble.transform.position.z; // Giữ nguyên trục Z

        // Di chuyển quả bóng theo trục X, giữ nó trong giới hạn
        float padding = 0.35f;
        Vector3 newPosition = currentBubble.transform.position;
        newPosition.x = Mathf.Clamp(mousePosition.x, leftLimit + padding, rightLimit - padding);
